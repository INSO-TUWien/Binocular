  ...
  return (
    <div className={'card w-96 bg-base-100 shadow-xl mb-3 mr-3 border-2 border-base-300'} key={props.dataPlugin.name}>
      <div className="card-body">
        ...
        {props.dataPlugin.requirements.file && state===State.unconfigured&& (<><label className="form-control w-full max-w-xs">
            <div className="label">
            ...
            <label>
              ...
              <input
                ...
                placeholder={'Name'}
              /></label>
            <button className={'btn btn-outline btn-xs mt-1'} onClick={() =>{
              ...
              if(fileInput&&fileNameInput) {
                if( fileInput.files && fileInput.files.length>0) {
                  ...
                  if (fileNameInput.value.length>0) {
                    ...
                    props.dataPlugin.init(undefined,undefined, {name:fileNameInput.value, file:file}).then(()=>{
        ...
        {props.dataPlugin.requirements.file && state===State.uploading&&(
          <div><span>Uploading Database</span><span className="loading loading-spinner loading-xs"></span>
          </div>)}
        {props.dataPlugin.requirements.file && state === State.configured&&(<div>
        ...
        <div className="card-actions justify-end">
          <button
            ...
            disabled={
              ...
              (props.dataPlugin.requirements.endpoint && endpointRef.current?.value.length === 0)||(props.dataPlugin.requirements.file && !(fileName && fileName.length >= 0))
            }
            onClick={() => {
              dispatch(
                addDataPlugin({
