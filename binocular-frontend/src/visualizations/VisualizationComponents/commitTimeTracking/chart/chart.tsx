...
export default (props: Props) => {
  const [commitChartData, setCommitChartData] = React.useState<
    CommitChartData[]
  >([]);
  ...
  const displayTooltip = (event, d, tooltip) => {
    ...
    const tooltipWidth = +tooltip
      .style("width")
      .substring(0, tooltip.style("width").length - 2);
    const tooltipHeight = +tooltip
      .style("height")
      .substring(0, tooltip.style("height").length - 2);
    tooltip
    ...
    commitChartData !== undefined && commitChartData.length > 0 ? (
      <CommitBarChart
        key={
          ...
          (props.useActualTime
            ? ""
            : commitChartData.map((d) => d.timeSpent.estimated).join("-")) +
          commitChartData
            .map((d) =>
              props.useRatio
                ? d.lineChanges /
                  Math.max(
                    d.timeSpent[props.useActualTime ? "actual" : "estimated"],
                    1,
                  )
                : d.lineChanges,
            )
...
const extractCommitData = async (props: Props): Promise<CommitChartData[]> => {
  ...
  const filteredCommits = props.filteredCommits.filter(
    (value, index, array) =>
      array.findIndex((el) => el.date === value.date) === index,
  );
  ...
  const sortedCommits = commitsWithDate.sort(
...
function addEstimatedTime(commits: any[], props: Props) {
  ...
  const mergedAuthors = [...props.mergedAuthors].filter((author) =>
    props.selectedAuthors.includes(author.mainCommitter),
  );
...
function filterCommits(commits: any[], props: Props) {
  ...
  const filteredCommits = commits.filter((c) => {
    ...
    const commitTime = props.useActualTime
      ? c.timeSpent.actual
      : c.timeSpent.estimated;
    if (
      commitTime < props.threshold.hours.lower ||
      commitTime > props.threshold.hours.upper
    ) {
      return false;
    }
    const lineChanges = c.stats.additions + c.stats.deletions;
    if (
      lineChanges < props.threshold.change.lower ||
      lineChanges > props.threshold.change.upper
    ) {
      return false;
    }
    if (
      lineChanges / (commitTime <= 0 ? 1 : commitTime) <
        props.threshold.ratio.lower ||
      lineChanges / (commitTime <= 0 ? 1 : commitTime) >
        props.threshold.ratio.upper
    ) {
      return false;
    }
    ...
    if (
      props.searchTerm &&
      !c.message.toLowerCase().includes(props.searchTerm.toLowerCase())
    ) {
      return false;
    }
