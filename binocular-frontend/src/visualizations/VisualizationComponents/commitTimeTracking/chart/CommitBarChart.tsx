...
interface State {
  ...
  displayTooltip: (
    event: any,
    d: any,
    window: d3.Selection<HTMLDivElement, unknown, null, any> | undefined,
  ) => void | undefined;
...
export default class CommitBarChart extends React.Component<Props, State> {
  ...
  loadChart() {
    ...
    const width =
      ...
      (this.state.content.commitData.slice(
        50 * this.state.page,
        50 + 50 * this.state.page,
      ).length /
        50.0);
    const nodeChartHeight = this.state.content.nodeChart ? 40 : 0;
    const numberOfBarcharts =
      this.state.content.upperChart && this.state.content.lowerChart ? 2 : 1;
    // 25 is for nav, 427 is the default value for a normal screen.
    const height =
      ((visualViewport?.height ?? 919) - nodeChartHeight - 25 - 20) /
      numberOfBarcharts;
    ...
    if (this.state.content.upperChart) {
      this.drawChart(
        ...
        width,
        height,
        { top: 40, right: 30, bottom: 0, left: 40 },
        false,
      );
    }
    ...
    if (this.state.content.lowerChart) {
      this.drawChart(
        ...
        width,
        height,
        { top: 0, right: 30, bottom: 40, left: 40 },
        true,
      );
    }
    ...
    this.drawNavigation(
      mainDiv,
      Math.ceil(this.state.content.commitData.length / 50.0),
    );
  }
  ...
  drawChart(
    ...
    const chartData = !showUpsideDown
      ? this.state.content.upperChart.slice(
          this.state.page * 50,
          50 + this.state.page * 50,
        )
      : this.state.content.lowerChart.slice(
          this.state.page * 50,
          50 + this.state.page * 50,
        );
    const svg = this.createSVG(
      mainDiv,
      !showUpsideDown ? "upperChart" : "lowerChart",
      margin,
      height,
    );
  ...
  drawNodes(
    ...
    const svg = this.createSVG(
      mainDiv,
      "nodeChart",
      { top: 5, right: 30, bottom: 5, left: 40 },
      40,
    );
    const x = this.createXAxis(
      this.state.content.nodeChart.slice(
        this.state.page * 50,
        50 + this.state.page * 50,
      ),
      width,
    ...
    if (this.state.displayTooltip) {
      ...
      nodes.on("click", (event, d) =>
        this.state.displayTooltip(
          event,
          this.state.content.commitData.find(
            (data) => data.date.toString() == d.ticks,
          ),
          tooltip,
        ),
      );
  ...
  drawNavigation(
    ...
    inputDiv
      ...
      .attr(
        "style",
        `text-align: center;font-size: 16px; width: 20; height: ${height}`,
      )
  ...
  appendBars(
    ...
    svg
      ...
      .attr("height", (d) =>
        !showUpsideDown ? height - y(d.barHeight) : y(d.barHeight),
      )
