...
export default class Bounds {
  static getBounds(db) {
    return Promise.all([findAll(db, 'stakeholders'), findAll(db, 'commits'), findAll(db, 'issues'), findAll(db, 'mergeRequests')]).then(
      (res) => {
        ...
        // first and last merge request
        res[3].docs = res[3].docs.sort((a, b) => {
          return new Date(a.date) - new Date(b.date);
        });

        response.firstMergeRequest = res[3].docs[0];
        response.lastMergeRequest = res[3].docs[res[3].docs.length - 1];
        return response;
      },
    );
  }
