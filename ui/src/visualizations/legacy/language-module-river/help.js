  ...
  <div className={cx('box', styles.help)}>
    ...
    <p>
      <ul>
        ...
        <li>
          ...
          commit can hold zero to many builds with several states. The build weight is taking the most occurring status and divides it by
          the given maximum amount of builds referring to this commit. Then the algorithm calculates the amount of changes of the current
          commit divided through the maximum amount of changes in the provided chart and multiplies it with the calculated build rate of the
          given commit. After that the result is added to or subtracted from the previous build rate of the last commit depending on the
          most accurate build status of the current commit.
        </li>
        <li>
    ...
    <p>
      <ul>
        ...
        <li>
          <i className="fa fa-filter" /> In the legend, you can filter the attributes that are displayed in the changes graph. The checkbox
          on the top selects/deselects all attributes of a category.
        </li>
        <li>
