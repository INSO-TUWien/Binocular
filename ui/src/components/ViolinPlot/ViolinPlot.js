...
export default class ViolinPlot extends React.Component {
  ...
  visualizeData() {
    ...
    const yScale = this.props.yScale; //Multiplication for the values on the y-scale
  ...
  getDimsAndPaddings(svg) {
    ...
    const node = !svg || typeof svg.node !== 'function' ? {} : svg.node();
   ...
   * @returns {{x: *, y: *}} d3 x and y scales. x scale as timescale, y scale as linear scale.
   */
