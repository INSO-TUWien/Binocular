...
class GatewayService {
  ...
  async configure(config) {
    ...
    const unregisterHandler = (requestHandler, callback) => {
      return Promise.resolve(unregister.bind(this)(requestHandler)).then(result => callback(null, result)).catch(error => callback(error));
    };
 ...
 * @returns {Promise<{signature: *}>}
 */
async function unregister(params) {
  const address = ipParser.exec(params.call.getPeer());
  ...
  this.services = this.services.filter(srv => !_.isEqual(srv, service));
}
