...
const log = require('debug')('service:gateway');
...
class GatewayService {
   ...
   */
  async start() {
    const port = this.server.bind(`0.0.0.0:${this.config.port}`, grpc.ServerCredentials.createInsecure());
  ...
  async disconnectService(service) {
    ...
    return new Promise((resolve, reject) =>
      service.comm.disconnect({ token: service.token }, response => (response instanceof Error ? reject(response) : resolve(response)))
    );
  }
  ...
  addServiceHandler(type, service) {
    if (this.register[type] === null && typeof service === 'function') {
...
function serviceTypesRegistration() {
  ...
  types.slice(1).forEach(type => (type && type.name ? (this.register[type.name] = null) : null));
}
...
function registration(params) {
  ...
  if (typeof this.register[service.type] === 'function') {
    this.register[service.type](service);
  }
