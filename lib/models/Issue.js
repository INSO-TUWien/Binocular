...
Issue.persist = function(_issueData) {
  ...
  delete issueData.project_id;

  return Issue.ensureById(issueData.id, issueData);
};
    ...
    .map(function(issuesPerAuthor) {
      return Stakeholder.findOneByGitlabId(issuesPerAuthor.author.id)
        ...
        .then(function(stakeholder) {
          ...
          return Promise.map(issuesPerAuthor.issues, issue =>
            stakeholder.connect(Issue.parse(issue))
          );
        });
    });
};
