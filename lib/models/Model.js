...
Model.define = function (name, options) {
  ...
  const ModelClass = class extends Model {
    ...
    static cursor(query) {
      if (!query) {
        return ModelClass.collection.all().then((rawCursor) => ModelClass.wrapCursor(rawCursor));
      } else {
        return ModelClass.rawDb.query(query).then((rawCursor) => ModelClass.wrapCursor(rawCursor));
      }
    }
