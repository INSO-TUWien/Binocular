...
Model.define = function(name, options) {
  ...
  const ModelClass = class extends Model {
    ...
    static findById(id) {
      ...
      return Promise.resolve(this.collection.lookupByKeys([id]).then(docs => docs[0]));
    }
    ...
    static cursor(query) {
      return Promise.resolve(ModelClass.rawDb.query(query)).then(rawCursor =>
        ModelClass.wrapCursor(rawCursor)
      );
    }
    ...
    save() {
      ...
      return Promise.resolve(
        ModelClass.collection.save(_.defaults({ _key: this._key }, this.data))
      ).then(function(resp) {
        self._key = resp._key;
        self._id = resp._id;
        self.isNew = false;
  ...
  _.each(attributes, function(attribute) {
    ...
    ModelClass[`findBy${Attr}`] = function(value) {
      ModelClass.log(`findBy${Attr}`, value);
      return Promise.resolve(ModelClass.collection.all({ [attribute]: value }));
    };

    ModelClass[`findOneBy${Attr}`] = function(value) {
      ModelClass.log(`findOneBy${Attr}`, value);
      return Promise.resolve(ModelClass.collection.firstExample({ [attribute]: value })).catch(
        { code: 404 },
        () => null
      );
    };

    ModelClass[`ensureBy${Attr}`] = function(value, data) {
      return ModelClass[`findOneBy${Attr}`](value).then(function(resp) {
        if (resp) {
          return ModelClass.parse(resp);
        } else {
