...
Model.define = function(name, options) {
  ...
  const ModelClass = class extends Model {
    ...
    static findAll() {
      return Promise.resolve(ModelClass.collection.all()).then(cursor => cursor.all()).map(d => ModelClass.parse(d));
    }
  ...
  _.each(attributes, function(attribute) {
    ...
    ModelClass.firstExample = function(data) {
      ...
      return Promise.resolve(ModelClass.collection.firstExample(data)).catch({ code: 404 }, () => null).then(d => ModelClass.parse(d));
    };
