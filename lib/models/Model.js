...
Model.define = function(name, options) {
  ...
  const ModelClass = class extends Model {
    ...
    static bulkCreate(datas) {
      ...
      return ModelClass.collection.import(datas).then(function() {
        return datas.map(data => new ModelClass(data, { isNew: false }));
      });
    }
  ...
  _.each(attributes, function(attribute) {
    ...
    ModelClass[`findOneBy${Attr}`] = function(value) {
      return Promise.resolve(ModelClass.collection.first({ [attribute]: value }));
    };

    ModelClass[`ensureBy${Attr}`] = function(value, data) {
      return ModelClass[`findBy${Attr}`](value).then(function(inst) {
        if (inst) {
          return inst;
        } else {
          inst = new ModelClass(data);
