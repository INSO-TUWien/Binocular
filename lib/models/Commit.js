...
const indexingBlacklist = require('../config.js').get().indexingBlacklist || [];
      ...
      async (filepath, parentCommitEntry, currentCommitEntry, commitFiles, parentCommitFiles) => {
        try {
          ...
          if (!(commitFiles.includes(filepath) || parentCommitFiles.includes(filepath))) {
            ...
            if (!indexingBlacklist.includes(filepath)) {
            ...
            //if this file is not blacklisted in the .binocularrc file
            //large files (where the number of changes is not important, such as package-lock.json)
            // should be blacklisted to improve performance.
            if (!indexingBlacklist.includes(filepath)) {
