  ...
  return Commit.findById(sha).then(function (instance) {
    ...
    return getBranchForCommit(sha).then((branch) => {
      ...
      return Commit.create(
        ...
        return Promise.all(
          Promise.map(nCommit.commit.parent, (parentSha) => {
 ...
 * @returns {*}
 */
Commit.prototype.processTree = async function (repo, nCommit, urlProvider, gateway) {
  return repo.getCommitChanges(nCommit.oid, nCommit.commit.parent[0], async (filepath, parentCommitEntry, currentCommitEntry) => {
    // ignore directories
    if (filepath === '.') {
      return;
    }
    ...
    // generate ids
    const Aoid = await currentCommitEntry.oid();

    const parentContent = Buffer.from(await currentCommitEntry.content()).toString('utf8');
    const currentContent = Buffer.from(await parentCommitEntry.content()).toString('utf8');
    const diff = jsdiff.structuredPatch(filepath, filepath, parentContent, currentContent);

    let additions = 0;
    let deletions = 0;
    let lineCount = currentContent.split('\n').length;
    ...
    for (const hunk of diff.hunks) {
      if (hunk.oldLines < hunk.newLines) {
        ...
        deletions += hunk.newLines;
      }
      hunks.push({
        webUrl: urlProvider ? urlProvider.getHunkUrl(Aoid, file, hunk.newStart, hunk.newLines) : null,
    ...
    return {
      file: file,
      languageContainer: file.detectLanguage(nCommit, gateway),
      lineCount: lineCount,
      stats: { additions: additions, deletions: deletions },
      hunks: hunks
    };
