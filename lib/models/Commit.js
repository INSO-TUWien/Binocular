...
Commit.prototype.processTree = function (repo, nCommit, urlProvider, gateway) {
  return Promise.cast(
    repo.getCommitChanges.bind(this)(nCommit.oid, nCommit.commit.parent[0], async (filepath, parentCommitEntry, currentCommitEntry) => {
      ...
      let parentContent = '';
      if (!(parentCommitEntry === null || (await parentCommitEntry.type()) === 'tree')) {
        parentContent = Buffer.from(await parentCommitEntry.content()).toString('utf8');
      }
      let currentContent = '';
      if (!(currentCommitEntry === null || (await currentCommitEntry.type()) === 'tree')) {
        currentContent = Buffer.from(await currentCommitEntry.content()).toString('utf8');
      }

      const dmp = new diff_match_patch();
      const tmpChars = dmp.diff_linesToChars_(parentContent, currentContent);
      const parentChars = tmpChars.chars1;
      const currentChars = tmpChars.chars2;
      const diff = dmp.diff_main(parentChars, currentChars);
      const patch_list = dmp.patch_make(diff);
      ...
      for (const hunk of patch_list) {
        ...
        hunks.push({
          webUrl: urlProvider ? urlProvider.getHunkUrl(currentOid, filepath, hunk.start2, hunk.length2) : null,
