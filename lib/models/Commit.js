  ...
  return Promise.resolve(nCommit.getTree()).then(function(tree) {
    return walkTree(tree.entries(), function(entry) {
      ...
      return Promise.join(
        ...
        const linesPerAuthor = {};

        console.log('processing', nCommit.sha());
        console.log('processing', n, 'hunks');

        for (let i = 0; i < n; i++) {
          const hunk = blame.getHunkByIndex(i);

          const sig = hunk.finalSignature();
          if (!(sig in linesPerAuthor)) {
            linesPerAuthor[sig] = 0;
          }

          linesPerAuthor[sig] += hunk.linesInHunk();
