...
Commit.persist = function(nCommit, urlProvider = null, owner, repoName) {
  const sha = nCommit.id().toString();
  const projectOwnerAndName = `${owner}/${repoName}`; // the fullName of the project the commit is in
  ...
  return Commit.findById(sha).then(function(instance) {
    if (!instance) {
      ...
      return Commit.create({
        ...
        date: nCommit.date(),
        author: nCommit.author().toString(),
        authorDate: new Date(nCommit.author().when().time() * 1000),
    ...
    // the commit is not persisted for the project {projectOwnerAndName}
    // only add the project to the projects array of the commit
    if (!instance.data.projects.includes(projectOwnerAndName)) {
      instance.data.projects.push(projectOwnerAndName);
      return Commit.create(instance.data, { isNew: false }).then((commit) => [commit, true]);
    }
    log('Skipped', sha);
