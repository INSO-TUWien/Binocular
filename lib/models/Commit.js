      ...
      .map(patch => {
        ...
        return Promise.join(
          ...
          Promise.resolve(nCommit.getEntry(newFile))
            ...
            .then(b => _.sumBy(b.toString(), ch => +(ch && ch.toString() === '\n')))
 ...
 */
Commit.prototype.processLanguage = function(repo, nCommit, gateway) {
  if (!gateway) {
  ...
  return Promise.all(
    Promise.cast(nCommit.getDiffWithOptions({ contextLines: 0 })).map(diff => {
      return Promise.cast(diff.patches()).map(patch => {
        const newFile = patch.newFile().path();

        return Promise.join(
          // get file
          File.ensureByPath(newFile).then(async file => {
            ...
            let languages = [];
            try {
              language.push(await new Promise((resolve, reject) =>
                service.comm.detectLanguages(
                  { token: service.token, path: newFile, content },
                  response => (response instanceof Error ? reject(response) : resolve(response))
                )
              ));
            } catch (error) {
