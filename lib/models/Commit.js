      ...
      async (filepath, parentCommitEntry, currentCommitEntry, commitFiles, parentCommitFiles) => {
        try {
          ...
          if (!(commitFiles.includes(filepath) || parentCommitFiles.includes(filepath))) {
            ...
            //file added (and not blacklisted in the .binocularrc file)
            ...
            //if it is a small file, use fast_myers_diff to compute diff
            if (parentContentLineArray.length < 4000) {
              ...
              //if it is a large file, use the git implementation to get the diff to increase performance
              const diffOutput = await new Promise((resolve) => {
                exec(
                  ` git diff --unified=0 ${nCommit.commit.parent[0]} ${nCommit.oid} -- ${filepath}`,
              ...
              //diff output also includes the change lines which we dont care about.
