...
Commit.prototype.processTree = async function (repo, nCommit, urlProvider, gateway) {
  repo.getCommitChanges(nCommit.oid, nCommit.commit.parent[0], async (filepath, currentCommitEntry, parentCommitEntry) => {
    // ignore directories
    if (filepath === '.') {
      return;
    }
    ...
    const AContent = Buffer.from(await currentCommitEntry.content()).toString('utf8');
    const BContent = Buffer.from(await parentCommitEntry.content()).toString('utf8');
    ...
    const diff = jsdiff.structuredPatch(filepath, filepath, AContent, BContent);
