...
Connection.define = function(FromModel, ToModel) {
  ...
  const ConnectionClass = class extends Connection {
    ...
    static ensure(data, fromTo) {
      this.log('ensure %o %o', { from: fromTo.from._id, to: fromTo.to._id }, data);
      data = data || {};
      return Promise.resolve(
        this.collection.firstExample({ _from: fromTo.from._id, _to: fromTo.to._id })
      )
        .then(exists => {
          this.log('Connection from %o to %o already exists', fromTo.from._id, fromTo.to._id);
          return [exists, false];
        })
        .catch({ code: 404 }, () => {
          this.log('Creating new connection');
          return this.create(data, fromTo).then(r => [r, true]);
        });
    }
  };
