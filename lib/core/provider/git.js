...
Repository.fromPath = function (currPath) {
  ...
  return Promise.resolve(isomorphicGit.init({ fs, dir: this.currPath || '.' })).then(() => {
...
Repository.prototype.getCommitChanges = async function (sha, parentSha, mapFunction) {
  const files = sha === undefined ? [] : await isomorphicGit.listFiles({ fs, dir: this.currPath || '.', ref: sha });
  const parentFiles = parentSha === undefined ? [] : await isomorphicGit.listFiles({ fs, dir: this.currPath || '.', ref: parentSha });
