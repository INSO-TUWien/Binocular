...
Repository.prototype.getFilePathsForBranch = async function (branchName) {
  try {
    if (branchName) {
      ...
      const result = await isomorphicGit.listFiles({ fs, dir: this.currPath || '.', ref: branchName });
      return result;
    }
  } catch (e) {
    log('error in getFilePathsForBranch: ' + e);
    return [];
  }
  return [];
};
...
Repository.prototype.getPreviousFilenames = async function (branchName, fileName) {
  if (branchName && fileName) {
    ...
    try {
      ...
      log('error in getPreviousFilenames: ', e);
      return [];
    }
  }
  return [];
...
Repository.prototype.getOwnershipForFile = async function (file, commit) {
  try {
    ...
    log('error in getOwnershipForFile: ', e);
    return {};
  }
};
