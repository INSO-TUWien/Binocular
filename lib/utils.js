...
module.exports = {
  ...
  getDbExport: async function () {
    ...
    exportJson.commits_files_stakeholders = await (
      await ctx.db.query('FOR i IN @@collection RETURN i', { '@collection': 'commits-files-stakeholders' })
    ).all();
  ...
  parseBlameOutput: function (output) {
    ...
    for (const line of lines) {
      ...
      if (line[0] === '\t') {
        ...
        const signature = author['author'] + ' ' + author['author-mail'];
