...
function get_imports_of_data(content,parser){
    ...
    const requireStatements = tree.rootNode.descendantsOfType('call_expression').filter((node) => node.children.length && node.children[0].text === 'require');

    for (const node of requireStatements) {
        const moduleName = node.children[1].text;
        importNames.push(moduleName.substring(2, moduleName.length -2));
    }
