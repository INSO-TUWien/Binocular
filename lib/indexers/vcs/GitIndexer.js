...
async function createOwnershipConnections(repo) {
  console.log('starting ownership connections');
  ...
  const commitFileConnections = await CommitFileConnection.findAll();
  ...
  for (const [commitId, cfcGroup] of commitFileConnectionsGrouped) {
    console.log(counter++ + '/' + commitFileConnectionsGrouped.length + ' commits');
