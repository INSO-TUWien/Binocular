  ...
  async index() {
    this.process = (async () => {
      ...
      for (const i in branches) {
        ...
        if (branches[i] !== 'HEAD') {
          const branch = { branchName: branches[i], id: i, currentActive: branches[i] === currentBranch };
      ...
      //await commits.reverse().map(async (commit) => await processCommit.bind(this)(commit));
      for (const commit of commits.reverse()) {
        await processCommit
          .bind(this)(commit)
          .catch({ stop: true }, () => null);
      }
      /*const commitIterator = commits.reverse().entries();
      let commit = commitIterator.next();
      while (!commit.done) {
        await processCommit
          .bind(this)(commit.value[1])
          .catch({ stop: true }, () => null);
...
async function processCommit(commit) {
  ...
  let connections = await commitDAO.processTree(this.repo, commit, this.urlProvider, this.gateway);
  return postProcessing.bind(this)(
    commitDAO,
    _.flatten(connections || []).filter((exist) => exist)
  );
}
