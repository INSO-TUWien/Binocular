...
const BranchFileConnection = require('../../models/BranchFileConnection.js');
...
async function createBranchFileConnections(repo) {
  ...
  for (const branch of branchesDAO) {
    ...
    const filePathsForBranch = await repo.getFilePathsForBranchRemote(name);
...
async function createOwnershipConnections(repo) {
  ...
  //also filter connections where files have been renamed without sprints
