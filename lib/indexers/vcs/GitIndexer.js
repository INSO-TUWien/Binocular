  ...
  async index() {
    this.process = (async () => {
      ...
      for (const branch of branchesDAO) {
        console.log(++counter + ' / ' + branchesDAO.length);
        const name = branch.data.branch;
        const filePathsForBranch = await this.repo.getFilePathsForBranch(name);
          ...
          filesDAO.map(async (file) => {
            if (filePathsForBranch.includes(file.data.path)) {
              ...
              return branch.ensureConnection(file).then(async (branchFile) => {
                //get previous filenames
                return this.repo.getPreviousFilenames(name, file.data.path).then(async (previousFilenames) => {
                  if (previousFilenames.length !== 0) {
                    //if there are previous filenames, we create a new connection
                    for (const prevFileObj of previousFilenames) {
                      const prevFilename = prevFileObj.fileName;
                      const dateOfRename = prevFileObj.timestamp;

                      if (file.data.path !== prevFilename) {
                        ...
                        BranchFileFileConnection.ensure({ dateOfRename: dateOfRename }, { from: branchFile, to: fileObj });
                      }
                    }
                  }
                });
