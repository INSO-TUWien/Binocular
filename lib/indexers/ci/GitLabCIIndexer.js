...
class GitLabCIIndexer extends BaseGitLabIndexer {
  ...
  index() {
    ...
    return this.getProject()
      .then(project => {
        return this.gitlab.getPipelines(project.id).on('count', count => this.reporter.setBuildCount(count)).each(pipeline => {
          ...
          return Build.findOneById(pipeline.id)
            .then(existingBuild => {
              if (!existingBuild || new Date(existingBuild.updatedAt).getTime() < new Date(pipeline.updatedAt).getTime()) {
                ...
                return Promise.join(this.gitlab.getPipeline(project.id, pipeline.id), this.gitlab.getPipelineJobs(project.id, pipeline.id))
