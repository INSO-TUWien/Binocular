...
function loadConfig() {
  ...
  const configuration = require('rc')(packageJson.name, {
    port: 48763,
    arango: {
      host: 'localhost',
      port: 8529
  ...
  configFiles.forEach(configFile => watchdog.watchFile(configFile, () => emit('updated', (config = loadConfig()))));

  if (configFiles.length < 1) {
    console.warn(`No ".${packageJson.name}rc" could be found in the home folder or in the following path "${process.cwd()}"!`);
  }
  process.chdir(cwd);
