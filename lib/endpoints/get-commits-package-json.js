...
module.exports = async function(req, res) {
    try{
        ...
        for(const commit of commitsPackage){
            ...
            commitDetail.devDependencies = await getDevDependencies(commitDetail.oid);
            response.push(commitDetail);
        }
        let evolution = [];
        if(dependency){
            for(const commit of response){
                evolution.push({version: commit.dependencies[dependency] ? commit.dependencies[dependency] : '-', date: commit.date, oid: commit.oid, message: commit.message, committer: commit.committer});
            }
