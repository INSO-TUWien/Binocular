...
module.exports = function (req, res) {
  ...
  // sort after date ascended
  cherryPickCommitInfos.sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  return (
    Promise.resolve()
      .then(() => {
        ...
        let otherRepoOriginUrl = new TextDecoder()
          .decode(execSync(`cd ${projectsPath}/${otherRepo} && git config --get remote.origin.url`))
          .trim();

        // create a remote if necessary
        gitUtils.createRemoteAndFetch(toRepoPath, toRepo, otherRepo, '', otherRepoOriginUrl);

        // check out the branch
        gitUtils.cleanRepoAndCheckOutBranch(toRepoPath, toBranch);

        return cherryPickCommits(cherryPickCommitInfos, toRepoPath, 0);
      })
      .then((result) => {
...
function cherryPickCommits(commitInfos, toRepoPath, index) {
  if (index < commitInfos.length) {
    ...
    try {
      ...
      commitInfos[index].conflictText = 'no conflicts';
    } catch (ex) {
      commitInfos[index].conflictText = 'conflicts (shown below)';
      // a conflict occurred
