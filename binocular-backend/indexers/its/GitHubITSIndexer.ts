    ...
    .then(async (url) => {
      ...
      return Promise.all([
        ...
        this.controller.getPullRequestsWithEvents(owner, repo).then((mergeRequests: ItsIssue[]) => {
          ...
          return mergeRequests.forEach((mergeRequest) => {
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-expect-error
            MergeRequest.findOneById(mergeRequest.id)
                ...
                if (
                  ...
                  return MergeRequest.persist({
                    ...
                    return persistedMergeRequest.save().then(() => {
                      log('Saved issue #' + persistedMergeRequest.data.iid);
                    });
                  });
                } else {
                  log('Skipping issue #' + mergeRequest.iid);
