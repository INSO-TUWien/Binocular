  ...
  module Service
    class RegistrationService
      ...
      def start
        @stop_mutex.with_write_lock { @stop = false }
        Thread.new do
          until @stop_mutex.with_read_lock { @stop }
            begin
      ...
      def register
        return unless @token_mutex.with_read_lock { @token.nil? }

        client = Binocular::Comm::Client.new(address: @config.data.dig('languageService', 'address'),
                                             port: @config.data.dig('languageService', 'port'))
        request = Binocular::Comm::RegistrationRequest.new(client: client,
                                                           type: Binocular::Comm::ServiceType::LanguageDetection)
      ...
      def heartbeat
        ...
        begin
          ...
          error = $ERROR_INFO
          # skip if error.code is connection refused
          @token_mutex.with_write_lock { @token = nil } if !error.nil? && error.code == 14
          raise $ERROR_INFO
        end
      end
    end
